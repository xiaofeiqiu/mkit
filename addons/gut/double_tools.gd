...
func is_stubbed_to_call_super(method_name, called_with):
	if stubber_ref.get_ref() != null:
		return stubber_ref.get_ref().should_call_super(
			double_ref.get_ref(), method_name, called_with
		)
	else:
		return false
...
func default_val(method_name, p_index):
	if stubber_ref.get_ref() == null:
		...
		var result = stubber_ref.get_ref().get_default_value(
			double_ref.get_ref(), method_name, p_index
		)
		return result
...
func get_singleton():
	...
	if to_return == null:
		push_error(
			"Trying to get a singleton reference on a non-singleton double: ",
			__gutdbl_values.singleton_name,
			"/",
			__gutdbl_values.singleton
		)
	return to_return
