...
func _ready():
	...
	if check_import == null:
		results_tree.add_centered_text(
...
func _apply_options_to_controls():
	...
	if _ctrls.run_externally_dialog.run_mode == _ctrls.run_externally_dialog.RUN_MODE_BLOCKING:
		...
		_ctrls.run_externally_dialog.run_mode == _ctrls.run_externally_dialog.RUN_MODE_NON_BLOCKING
	):
		mode_ind = "ExN"
...
func _save_user_prefs():
	...
	_user_prefs.run_externally.value = (
		_ctrls.run_externally_dialog.run_mode != _ctrls.run_externally_dialog.RUN_MODE_EDITOR
	)
...
func _save_config():
	...
	if w_result != OK:
		push_error(
			str(
				"Could not write options to ",
				GutEditorGlobals.editor_run_gut_config_path,
				": ",
				w_result
			)
		)
...
func _run_externally():
	...
	_shell_out_panel.additional_arguments = (
		_ctrls.run_externally_dialog.get_additional_arguments_array()
	)
...
func _apply_shortcuts():
	...
	_ctrls.run_button.shortcut = _ctrls.shortcut_dialog.scbtn_run_all.get_shortcut()
