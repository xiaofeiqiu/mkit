...
func _validate_and_confirm():
	if validate_arguments():
		...
		var dlg_text = str(
			"Invalid arguments.  The following cannot be used:\n", " ".join(_invalid_args)
		)
...
func _add_controls():
	...
	_btn_blocking = _add_run_mode_button(
		"Externally - Blocking",
		button_desc,
		(
	...
	_btn_non_blocking = _add_run_mode_button(
		"Externally - NonBlocking",
		button_desc,
		(
			"Debugger is not enabled, and cannot be enabled.  All output (print, errors, warnings, etc) "
			+ "appears in the GUT panel, and [b]not[/b] the Output or Debugger panels.  \n"
	...
	_txt_additional_arguments = opt_maker.add_value(
	...
	_add_blurb(
		(
			"[b]Be Careful[/b]  There are plenty of argument combinations that may make this "
	...
	_add_blurb(
		"You can use these buttons to get a list of valid GUT and Godot options.  They print the CLI help text for each to the [b]Output Panel[/b]."
	)
	_btn_godot_help = Button.new()
	_btn_godot_help.text = "Print Godot CLI Help"
	_main_container.add_child(_btn_godot_help)
	_btn_godot_help.pressed.connect(func(): await _show_help("get_godot_help"))

	_btn_gut_help = Button.new()
	_btn_gut_help.text = "Print GUT CLI Help"
	_main_container.add_child(_btn_gut_help)
	_btn_gut_help.pressed.connect(func(): await _show_help("get_gut_help"))
...
func _load_from_config_file(f):
	...
	opt_maker.controls.additional_arguments.value = f.get_value("main", "additional_arguments", "")
...
var _invalid_args = ["-d", "--debug", "-s", "--script", "-e", "--editor"]
var _invalid_blocking_args = ["--headless"]
