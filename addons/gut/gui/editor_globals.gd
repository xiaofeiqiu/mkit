...
static var editor_run_gut_config_path = "gut_editor_config.json":
	...
	get:
		return temp_directory.path_join(editor_run_gut_config_path)
...
static var editor_run_bbcode_results_path = "gut_editor.bbcode":
	get:
		return temp_directory.path_join(editor_run_bbcode_results_path)
	set(v):
		pass

static var editor_run_json_results_path = "gut_editor.json":
	get:
		return temp_directory.path_join(editor_run_json_results_path)
	set(v):
		pass

static var editor_shortcuts_path = "gut_editor_shortcuts.cfg":
	get:
		return temp_directory.path_join(editor_shortcuts_path)
	set(v):
		pass

static var run_externally_options_path = "gut_editor_run_externally.cfg":
	get:
		return temp_directory.path_join(run_externally_options_path)
	set(v):
		pass
...
static var user_prefs = _user_prefs:
	...
	get:
		if _user_prefs == null and Engine.is_editor_hint():
			...
			# for EditorInterface.
			_user_prefs = GutUserPreferences.new(
				GutUtils.get_editor_interface().get_editor_settings()
			)
		return _user_prefs
