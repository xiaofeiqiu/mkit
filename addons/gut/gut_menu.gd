...
func add_menu(display_text, sig_to_emit, tooltip = ""):
	...
	_menus[sig_to_emit.get_name()] = {index = index, id = index, callback = sig_to_emit.emit}
...
func make_menu():
	...
	add_menu("Run All", run_all, "Run all tests")
	add_menu("Run Script", run_script, "Run the currently selected script")
	add_menu("Run Inner Class", run_inner_class, "Run the currently selected inner test class")
	add_menu("Run Test", run_test, "Run the currently selected test")
	add_menu(
		"Run At Cursor",
		run_at_cursor,
...
func set_shortcut(menu_name, accel_or_input_key):
	if typeof(accel_or_input_key) == TYPE_INT:
		...
		sub_menu.set_item_accelerator(
			_menus[menu_name].index, accel_or_input_key.get_keycode_with_modifiers()
		)
...
func apply_gut_shortcuts(shortcut_dialog):
	set_shortcut("show_gut", shortcut_dialog.scbtn_panel.get_input_event())
	set_shortcut("run_all", shortcut_dialog.scbtn_run_all.get_input_event())
	set_shortcut("run_script", shortcut_dialog.scbtn_run_current_script.get_input_event())
	set_shortcut("run_inner_class", shortcut_dialog.scbtn_run_current_inner.get_input_event())
	set_shortcut("run_test", shortcut_dialog.scbtn_run_current_test.get_input_event())
	set_shortcut("run_at_cursor", shortcut_dialog.scbtn_run_at_cursor.get_input_event())
	set_shortcut("rerun", shortcut_dialog.scbtn_rerun.get_input_event())
