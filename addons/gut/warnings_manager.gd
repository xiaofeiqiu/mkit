...
const WARNING_LOOKUP = {IGNORE: "IGNORE", WARN: "WARN", ERROR: "ERROR"}
...
static func replace_warnings_values(dict, replace_this, with_this):
	...
	for key in to_return:
		if (
			typeof(to_return[key]) == TYPE_INT
			and (replace_this == -1 or to_return[key] == replace_this)
		):
...
static func load_script_using_custom_warnings(
	path: String, warnings_dictionary: Dictionary
) -> Variant:
