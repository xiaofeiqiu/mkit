...
| 世界 | 在传送门区域内进出 Field（田野） | **G** |
| 世界 | 在门口区域内进出 Elder Room（村长室） | **R** |
| 世界 | 在洞口区域内进入 Trial Cave（试炼序列） | **C** |
| NPC | 与长老对话 / 推进对话 | **T** |
| NPC | 向长老请求祝福 | **Y** |
| NPC | 接取人工任务 | **M** |
| 物品 | 在 Village Supply 区域内购买药水 | **B** |
| 物品 | 在 Village Supply 区域内出售兽爪 | **V** |
| 物品 | 使用药水 | **H** |
| 物品 | 装备 / 卸下 Field Blade | **E** |
| 试炼 | 选择奖励 1 / 2 / 3 | **1** / **2** / **3** |
| 存档 | 保存 / 读取 | **F6** / **F7** |
| 平台 | 触发 rewarded revive 示例 | **N** |
| 平台 | 在 Village Supply 区域内触发 gold pack 示例 | **P** |
| 云存档 | 保存 / 读取 cloud slot 示例 | **O** / **U** |
| 调试 | 即时击败田野野兽 | **K** |
...
2. **前往田野**：走到 Field Gate 区域内按 **G** 穿越传送门进入 Field。用 **WASD** 靠近 Field Beast，**Space** 近战或 **Q/F** 施放 Firebolt。Firebolt 会先造成伤害并施加 burn，右侧 HUD 会显示 Field Beast 的 HP / burn 状态；击杀后敌人会从画面移除，任务目标自动推进并掉落兽爪。（调试快捷键：**K** 直接击败野兽跳过战斗）

3. **返回村庄**：走到 Village Gate 区域内按 **G** 返回村庄。按 **T** 再次与长老对话，交还任务领取奖励，可选择接受祝福（**Y**）。

4. **商店操作**（可选）：对话结束后商店解锁。走到 Village Supply 区域内按 **B** 购买药水，**V** 出售兽爪。若背包里有 Field Blade，按 **E** 装备，属性立即生效。按 **H** 使用药水恢复血量。

5. **进入试炼**：在 Field 走到 Trial Cave 洞口区域内按 **C**，触发三房间序列（RunDirector）。每个房间清除所有敌人后弹出奖励界面，按 **1**、**2** 或 **3** 选择奖励，奖励效果立即应用。三个房间全部通过即完成试炼。
