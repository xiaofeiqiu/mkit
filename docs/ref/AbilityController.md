...
AbilityController 是实体释放技能的控制器。它管理技能实例、检查条件/消耗/冷却、启动施法、执行效果并发出事件。玩家和敌人都可以放技能，应该共享同一条释放链路。

## 设计目的

成为技能释放的单一协调者，把"能否施放"的所有检查（冷却、资源、条件）和"如何施放"的流程（即时 vs 施法读条、效果执行、冷却启动）集中管理，使状态机只需决定进入 CastAbilityState，具体执行细节全由 AbilityController 处理。
...
- **`cast(ability_id, context)`**：主要执行入口。通过全部检查后：扣除资源、发出 `ability_cast_started`，若 `cast_time > 0` 启动 CastAction（读条），否则立即执行效果并启动冷却。
