...
- **quest_turned_in**：奖励 effects 成功后发出。
...
- **`turn_in_quest(quest_id, context)`**：执行 reward_effects，全部成功后标记 turned_in，repeatable 任务回到 available；奖励失败时保持 completed 并返回 false。
