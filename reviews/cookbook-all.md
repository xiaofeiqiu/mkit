...
**顺带发现（需要决策，不只是补文档）：** 两个字段在 mkit 内部没有任何消费者：

- `AbilityDefinition.range` — 已决定删除；施放距离统一用 `conditions` + `TargetInRangeCondition.range`
- `EntityDefinition.loot_table_id` — 没有代码读它（掉落走 `RoomDefinition.reward_pool_ids`）

文档无法解释「mkit 拿它做什么」。`AbilityDefinition.range` 已按 API 收敛原则删除，避免和 `TargetInRangeCondition.range` 形成两套路由；`EntityDefinition.loot_table_id` 仍需在「标注预留 / 接消费者 / 删除」之间单独决策。
...
- `EntityDefinition.loot_table_id` — **标注预留**（见第一节决策）
...
| 决策 | `EntityDefinition.loot_table_id` 标注预留 or 接消费者 or 删除 | 单独 issue |
